...
namespace MultiplayerSFS.Common
{
    ...
    public class WorldState
    {
        ...
        public WorldState(string path)
        {
            ...
            string persistentPath = Path.Combine(folderPath, "Persistent");
            ...
            string worldSettingsPath = Path.Combine(folderPath, "WorldSettings.txt");
            if (!File.Exists(worldSettingsPath))
                throw new Exception("'WorldSettings.txt' file cannot be found or could not be loaded.");
            
            WorldSettings settings = JsonConvert.DeserializeObject<WorldSettings>(File.ReadAllText(worldSettingsPath));
            
            string worldStatePath = Path.Combine(persistentPath, "WorldState.txt");
            if (!File.Exists(worldStatePath))
                throw new Exception("'WorldState.txt' file cannot be found or could not be loaded.");
            
            WorldSave.WorldState state = JsonConvert.DeserializeObject<WorldSave.WorldState>(File.ReadAllText(worldStatePath));
            
            string rocketsPath = Path.Combine(persistentPath, "Rockets.txt");
            if (!File.Exists(rocketsPath))
                throw new Exception("'Rockets.txt' file cannot be found or could not be loaded.");
            
            List<RocketSave> rocketSaves = JsonConvert.DeserializeObject<List<RocketSave>>(File.ReadAllText(rocketsPath));
